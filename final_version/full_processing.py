...
def get_dataframe(filename):
    ...
    x_color,y_color = find_peaks(color_plot)
    ...
    volt = list(df[1])
    r2 = get_r2(cvs, volt)
    rise_times,starts = get_rise_times(color_plot, x_color, y_color)
    ...
    #starts = get_start_and_end(color_plot, x_color, y_color)
...
def test_get_dataframe(filename):    
    x_color, y_color = find_peaks(filename)
    color_plot = read_tdms(filename)
    start_end = get_start_and_end(color_plot, x_color, y_color)

    cvs = get_cvs(color_plot, x_color,y_color)
    df_sample = pd.read_csv('sample_volt.txt', header=None, delimiter='\t')
    volt = list(df_sample[1])
    r2 = get_r2(cvs, volt)
